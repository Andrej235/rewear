...
public partial class DeliveryBoxController
{
    ...
    [Authorize(Roles = Roles.Admin)]
    [HttpPatch("admin/{boxId:guid}/items/status")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult> UpdateBoxItemsStatusBulk(
        [FromRoute] Guid boxId,
        [Required] [FromQuery] InventoryItemStatus status
    )
    {
        var result = await service.UpdateItemsStatusBulk(boxId, status);
